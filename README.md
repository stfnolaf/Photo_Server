...
The same spec drives the checked-in web client (`frontend/web/src/api/generated`, from `@hey-api/openapi-ts`), whose drift check runs in `run_all_tests.sh`. Python consumers in the server's own package skip codegen and import the models directly: the `photo-upload` CLI validates every JSON exchange with the API through `photo_server.api_schemas` — the very classes the routes validate with and the source of the spec — so a server response that drifts from the spec fails loudly as a contract violation instead of as a `KeyError` mid-upload, and no second model family can drift from the server's. The CLI's transport is a small hand-written httpx client (its PUT sends a raw binary body to the per-file batch endpoint).
